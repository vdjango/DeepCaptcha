...
class TrainModel(object):
    ...
    def __init__(self, pretrained=False, model_path=None, requires_grad=False, *args, **kwargs):
        self.model_save = './model'
        self.data_url = './data/verification/'
        ...
        self.data_transforms = {
            'train': transforms.Compose([
                ...
                transforms.RandomGrayscale(),  # 以p的概率将图像随机转换为灰度
        ...
        self.net = RNet(pretrained=pretrained, requires_grad=requires_grad)
        if self.model_path:
        ...
        output_params = list(map(id, self.net.fc.parameters()))
        feature_params = filter(lambda p: id(p) not in output_params, self.net.parameters())
        params = [
            {'params': feature_params},
            {'params': self.net.fc.parameters(), 'lr': self.lr * 100}
        ]

        self.optimizer = optim.Adam(params, lr=self.lr, weight_decay=self.weight_decay)
        # self.optimizer = optim.Adam(self.net.parameters(), lr=self.lr, weight_decay=0.001)
        self.exp_lr_scheduler = lr_scheduler.StepLR(self.optimizer, step_size=self.step_size, gamma=self.gamma)
...
if __name__ == '__main__':
    path = 'llll'  # os.path.join('model/best/epoch_610_acc_0.47682927_loss_2.07039697.pt')
    model_path = None
    pretrained = True  # 预训练网络
    requires_grad = True  # False冻结网络，True不冻结网络

    if os.path.exists(path):
        model_path = path
        # requires_grad = True

    train = TrainModel(pretrained=pretrained, model_path=model_path, requires_grad=requires_grad)
    train()
